...
_STATION_BY_NAME["price edward"] = "PRE"  # tolerate the data-source typo
...
def fetch_next_train(line: str, station: str) -> dict:
    line_code = _resolve(line, MTR_LINES, _LINE_BY_NAME, "line")
    sta_code = _resolve(station, MTR_STATIONS, _STATION_BY_NAME, "station")

    r = httpx.get(NEXT_TRAIN_URL, params={"line": line_code, "sta": sta_code}, timeout=15)
    r.raise_for_status()
    body = r.json()
...
def fetch_mtr_bus_schedule(route: str, station_id: str, language: str = "en") -> dict:
    payload = {
        "language": language,
